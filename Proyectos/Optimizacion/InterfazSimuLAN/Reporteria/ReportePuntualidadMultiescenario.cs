...
{
    /// <summary>
    /// Reporte de puntualidad multiescenario
    /// </summary>
    internal class ReportePuntualidadMultiescenario:ReportBuilder
    {
        #region CONSTANTS
        ...
        /// <summary>
        /// Matriz con los intervalos de confianza por escenarios simulado
        /// </summary>
        private Dictionary<int, double[,]> _desvest;

        /// <summary>
        /// Matriz con los promedios por escenarios simulado
        /// </summary>
        private Dictionary<int, double[,]> _promedios;
        ...
        /// <summary>
        /// Diccionario con los valores a usados para construir el reporte(int escenario, int replica, int estándar, double puntualidad)
        /// </summary>
        private Dictionary<string, Dictionary<int, Dictionary<int, double>>> _valores_reporte;
        ...
        public ReportePuntualidadMultiescenario(string path, string filename, string[] sheetsNames,List<int> stds, Dictionary<string,Dictionary<int, Dictionary<int, double>>> valoresReporte)
            : base(path, filename, sheetsNames)
        {
            ...
            this._stds = stds;
            this._promedios = new Dictionary<int, double[,]>();
            this._desvest = new Dictionary<int, double[,]>();
            foreach (int std in stds)
            {
                _promedios.Add(std, new double[NUM_ESCENARIOS, NUM_ESCENARIOS]);
                _desvest.Add(std, new double[NUM_ESCENARIOS, NUM_ESCENARIOS]);
            }
            ConstruirPromedios();
            ConstruirDesvest();
        }
        ...
        public override void CrearReporte(string titulo, bool juntaTitulos)
        {
            ...
            foreach (int estandar in _stds)
            {
                ...
                ImprimirValores(sheet, _primera_fila, _primera_columna, _promedios[estandar]);
                ImprimirTabla(sheet, _primera_fila + 8, _primera_columna, "Desvest");
                ImprimirValores(sheet, _primera_fila + 8, _primera_columna, _desvest[estandar]);
                ImprimirConsolidado(sheet, _primera_fila + 5, _primera_columna, "Desvest consolidada", _promedios[estandar]);
        ...
        private void ConstruirDesvest()
        {
            foreach (string key in _valores_reporte.Keys)
            {
                ...
                int nReplicas = _valores_reporte[key].Count;
                foreach (int std in _stds)
                {
                    ...
                    else
                    {
                        ...
                        {
                            promedio += _valores_reporte[key][i][std];
                        }
                        promedio /= nReplicas;
                        for (int i = 0; i < nReplicas; i++)
                        {
                            varianza += Math.Pow(Math.Abs(_valores_reporte[key][i][std] - promedio), 2);
                        }
                        varianza /= (nReplicas - 1);
                    }
                    _desvest[std][indexClima - 1, indexMantto - 1] = Math.Sqrt(varianza);
                }
            }
        }
        ...
        private void ConstruirPromedios()
        {
            foreach (string key in _valores_reporte.Keys)
            {
                ...
                foreach (int std in _stds)
                {
                    ...
                    promedio /= nReplicas;
                    _promedios[std][indexClima - 1, indexMantto - 1] = promedio;
                }
            }
        }
